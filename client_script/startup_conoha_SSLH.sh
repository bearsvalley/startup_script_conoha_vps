...
#------------------------------------------------
PASSWORD='darkqueen' 
#[ x${PASSWORD} == 'x' ] && PASSWORD=`cat /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 12 | head -n 1`
...
#
logging_INFO " Start update. Sometimes this takes long time as 5~10min... " &&\
    yum -q -y makecache &&\
    yum -q -y upgrade  &&\
...
# ssh -p 443 
sed -e "s/#Port\s*22 */Port ${SSH_PORT}/g" -i /etc/ssh/sshd_config &&\
 logging_INFO "End edit sshd_config "
...
logging_INFO "starting doker damon..." &&\
